...
// BMS variables
BMS* bms = new BMS(&Serial5, 30);
//byte bms_request[] = {0xDB,0xDB,0x00,0x00,0x00,0x00};
byte bms_request[] = {0x5A,0x5A,0x00,0x00,0x00,0x00};
...
void loop()                                                            
{
    ...
    InsertV(&CurrentDataFrame, "Brake", &BrakeOnint);

    InsertV(&CurrentDataFrame, "AnalogBrake", &BrakeIn,2);

    InsertV(&CurrentDataFrame, "CCOn", &CCOnint);

    InsertV(&CurrentDataFrame, "CCArm", &CCSetint);

    InsertV(&CurrentDataFrame, "HVOn", &HVOnint);

    InsertV(&CurrentDataFrame, "daylightOn", &daylightOnint);

    InsertV(&CurrentDataFrame, "hazardOn", &hazardOnint);

    InsertV(&CurrentDataFrame, "PV1On", &PV1Onint);

    InsertV(&CurrentDataFrame, "PV2On", &PV2Onint);
  ...
  if(Serial4.availableForWrite() >= 20){
    ...
    if(Serial4.available() >= 9){
      Serial4.readBytes(contrl, 9);
      ...
      CCSet = ((contrl[8] << 6));
      HVOn = ((contrl[8] << 4));
      daylightOn = ((contrl[8] << 3));
      hazardOn = ((contrl[8] << 2));
      PV1On = ((contrl[8] << 1));
      PV2On = (contrl[8] >> 7);

      BrakeOnint = BrakeOn >> 7,
      CCOnint = CCOn >> 7,
      CCSetint = CCSet >> 7,
      HVOnint = HVOn >> 7, 
      daylightOnint = daylightOn >> 7, 
      hazardOnint = hazardOn >> 7, 
      PV1Onint = PV1On >> 7,
      PV2Onint = PV2On;
