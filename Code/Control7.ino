...
void loop() {
  ...
  if(Serial4.availableForWrite() >= 8){
...
void CC(unsigned long i) {
  if(CCON && debounce(millis(), prevCCUpd, 200)){
    CCSET = !CCSET;
    prevCCUpd = millis();
    if(CCSET && CCstatus){
      CCVmot = outPedalV;
