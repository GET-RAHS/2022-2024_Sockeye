...
void loop()                                                            
{
    ...
    InsertV(&CurrentDataFrame, "CCArm", &CCSet);

    InsertV(&CurrentDataFrame, "HVOn", &HVOn);

    InsertV(&CurrentDataFrame, "daylightOn", &daylightOn);

    InsertV(&CurrentDataFrame, "hazardOn", &hazardOn);

    InsertV(&CurrentDataFrame, "PV1On", &PV1On);

    InsertV(&CurrentDataFrame, "PV2On", &PV2On);
  ...
  if(Serial4.availableForWrite() >= 20){
    ...
    if(Serial4.available() >= 9){
      ...
      BrakeOn = ((contrl[8] << 7) >> 7);
      CCOn = ((contrl[8] << 6) >> 7);
      CCSet = ((contrl[8] << 5) >> 7);
      HVOn = ((contrl[8] << 4) >> 7);
      daylightOn = ((contrl[8] << 3) >> 7);
      hazardOn = ((contrl[8] << 2) >> 7);
      PV1On = ((contrl[8] << 1) >> 7);
      PV2On = (contrl[8] >> 7 );
